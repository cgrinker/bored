...
- **Snapshot guard plumbed into retention**: `TransactionManager` computes the oldest snapshot read LSN across active transactions, feeds it into `WalCommitPipeline`, and pins that guard alongside the durability horizon so `WalRetentionManager` keeps segments needed by long-running readers while updated Catch2 suites verify the hand-off.
