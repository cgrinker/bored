...
- **Relevant code**: `src/planner` (logical → physical planning), `src/executor` (operator pipeline), `src/shell/shell_backend.cpp` (storage-backed shell tables), telemetry hooks in `include/bored/storage/storage_telemetry_registry.hpp`.
- **Action items**
...
## Milestone 3 — Persistent Storage Backend _(Status: Complete)_
...
- **Action items**
    ...
    2. [x] Extend CLI/config (`src/tools/bored_shell_main.cpp`) to accept database directories, WAL retention knobs, and IO_uring settings, and pass them to the backend.
	- 2025-11-02: Backend honors persistence when directories are supplied programmatically; CLI flag plumbing moved to this milestone to keep CLI parity with programmatic configuration.
	- 2025-11-01: `run_shell_cli` now threads CLI flags for storage directories, WAL retention, and async IO through `ShellBackend::Config`, and `main` delegates to the reusable entry point for future harness coverage.
