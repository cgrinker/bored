...
bool parse_string_literal(std::string_view text, std::size_t& position, std::string& out, std::string& error)
{
    if (position >= text.size() || text[position] != '\'') {
        error = "Expected string literal starting with a single quote.";
        return false;
    }
    ++position;
    while (position < text.size()) {
...
bool parse_identifier_list(std::string_view text, std::vector<std::string>& out)
{
    ...
    while (position < text.size()) {
        ...
        std::size_t token_begin = position;
        while (position < text.size() && text[position] != ',') {
            ++position;
        }
        auto token = trim_copy(text.substr(token_begin, position - token_begin));
        if (token.empty()) {
            return false;
        }
...
bool is_signed_integer_literal(std::string_view token)
{
    ...
    for (; position < token.size(); ++position) {
        if (std::isdigit(static_cast<unsigned char>(token[position])) == 0) {
            return false;
        }
        digit_seen = true;
    }
...
InsertParseResult parse_insert(std::string_view input)
{
    ...
    std::smatch match;
    if (!std::regex_match(statement_text, match, kInsertPattern)) {
        ParserDiagnostic diagnostic{};
        diagnostic.severity = ParserSeverity::Error;
    ...
    for (const auto& row : statement.rows) {
        if (row.values.size() != statement.columns.size()) {
            ParserDiagnostic diagnostic{};
            diagnostic.severity = ParserSeverity::Error;
            diagnostic.message = "VALUES row does not match the column list";
            diagnostic.statement = trim_copy(input);
            diagnostic.remediation_hints =
                {"Ensure each VALUES row has the same number of entries as the column list."};
            result.diagnostics.push_back(std::move(diagnostic));
            result.statement = nullptr;
            result.arena.reset();
            return result;
...
UpdateParseResult parse_update(std::string_view input)
{
    ...
    if (std::regex_match(rhs_text, op_match, kSetOperationPattern)) {
        ...
        auto& binary = make_binary_expression(result.arena,
                                              (op_token == "+") ? relational::BinaryOperator::Add
                                                                  : relational::BinaryOperator::Subtract,
                                              left,
                                              right);
    ...
    } else if (iequals(rhs_text, "NULL")) {
        assignment.value = &make_null_literal_expression(result.arena);
    } else {
        assignment.value = &make_identifier_expression(result.arena, rhs_text);
    }
