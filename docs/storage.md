...
- **Commit pipeline scaffolded**: `TransactionManager` now stages commits through a pluggable pipeline with explicit `FlushingWal`/`Durable`/`Publishing` states, wiring in-flight contexts for durability acknowledgements ahead of WAL writer integration.
