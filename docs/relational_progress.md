...
6. **Comprehensive Transactions & Isolation Levels (Planned)**
   ...
   2. [ ] Add multi-version concurrency control (MVCC) visibility rules across executor operators (delivers snapshot isolation semantics across scans, joins, and DML). _MVCC visibility regression coverage for sequential scans landed; extend to joins, spools, and DML targets next._
