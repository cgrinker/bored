...
Latest validation: `ctest --output-on-failure` (486/486) on 2025-11-05 covering greedy join reorder tracing, recursive spool selection, multi-statement worktable reuse, advanced index catalog metadata propagation, view accessor/introspection expansion, and the new index-costing regression suite; `build/bored_benchmarks --samples=10 --json` refreshed FSM refresh, retention pruning, overflow replay, and spool recovery baselines on 2025-11-05; targeted `ctest -R "TransactionManager|ShellBackend"` suites now exercise configurable isolation defaults, BEGIN parsing, telemetry wiring, and conflict counters (added 2025-11-07).
...
- **View Metadata Surfacing**: Catalog accessor caches now materialise persisted view definitions alongside table descriptors, enabling introspection snapshots and diagnostics to enumerate CREATE VIEW payloads for planner, binder, and shell consumers.
...
7. **Extended SQL Surface & Tooling (In Progress)**
   ...
   - [x] Extend catalog accessor/introspection layers to list views alongside tables so planner/binder and shell introspection can discover stored definitions.
   - **Milestone 7 (Views) — current status:** WAL/catalog plumbing, accessor caches, introspection JSON, and unit coverage (`tests/catalog_view_accessor_tests.cpp`, `tests/catalog_introspection_tests.cpp`) are landed. Next actions are to expose view listings through bored_shell meta commands (target `\rv`) and parser/binder DROP VIEW wiring, then add end-to-end shell regression once metadata commands ship.
